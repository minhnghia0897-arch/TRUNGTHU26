...
Data-driven hoàn toàn. Money trên `web_order` luôn ở **tiền tệ người đặt**.
...
customer         { id, name, phone, region(vn|kr),           -- region người đặt
                   pancake_customer_id, pancake_shop, created_at }

web_order        { id, customer_id, buyer_region(vn|kr),
                   currency(vnd|krw), subtotal, shipping_total, grand_total,
                   payment_status(pending|paid|canceled), transfer_code,
                   created_at }
...
order_line       { id, web_order_id, recipient_id,           -- dòng này giao cho AI
                   kind(box|combo|la), box_id?, flavors(jsonb)?, qty,
                   unit_price, line_total }                   -- money ở currency người đặt

shipment         { id, web_order_id, recipient_id,
                   fulfillment_region(vn|kr), warehouse_id,
                   shipping_fee, shipping_mode,
...
Quan hệ: `web_order 1—N recipient`, `recipient 1—1 shipment`, `order_line N—1 recipient`. Mỗi `shipment` map tới đúng **1 đơn Pancake**.
...
- **Giá hộp tự chọn** = giá phẳng theo size hộp (theo bảng giá vùng người đặt) `+ Σ phụ thu vị premium`. KHÔNG cộng giá từng bánh.
- **Combo** tính như hộp tự chọn với vị cố định. **Mua lẻ** = `flavor.price[buyer_region] × qty`.
- **Caveat biên — ca chéo vùng #4** (người đặt VN trả giá VN thấp nhưng giao từ kho Hàn chi phí cao): bù bằng phí ship/handling giao-tại-Hàn ở §6, không bẻ giá SP. Ca này hiếm (thị trường chính ở Hàn) nên chấp nhận được.
- Làm tròn mọi số: VND `toLocaleString('vi-VN')+'đ'`, KRW `'₩'+toLocaleString('en-US')`.
...
- **Ca chéo vùng** (tiền tệ người đặt ≠ tiền tệ kho): phí ship của kho ở tiền tệ local → **quy đổi về tiền tệ người đặt theo tỉ giá cấu hình** `fx_rate(krw↔vnd)`. Lưu tỉ giá ở config, không hardcode.
- `shipping_total = Σ shipping_fee (đã quy đổi về currency người đặt)`.
...
- `grand_total = subtotal + shipping_total`, tất cả ở tiền tệ người đặt. Một bill, một mã CK, dù nhiều kiện nhiều vùng.
...
4. **Xem lại** — bill theo tiền tệ người đặt, tách phí ship từng kiện, ghi rõ kiện nào đi kho nào.
5. **Thanh toán** — hiện STK theo tiền tệ + mã CK duy nhất.
6. **Xác nhận** → đẩy Pancake → chuyển sang trang theo dõi.

Giỏ và trạng thái đơn giữ ở server/Supabase, KHÔNG localStorage.
...
- STK nhận tiền theo tiền tệ: tài khoản VN cho VND, tài khoản Hàn cho KRW (config theo `currency`).
...
- Khi tiền về, đánh dấu `payment_status = paid` (đối soát tay, hoặc nối cổng tự động sau — để ngỏ).
- Bill hiển thị: dòng hàng + phí ship từng kiện + tổng, một tiền tệ.
...
### 10.2 Đẩy đơn — 1 web_order → N đơn POS
- Kích hoạt khi `payment_status = paid`.
- Với **mỗi `shipment`**: tạo 1 đơn POS ở **kho/shop khớp `fulfillment_region`** (kho VN hoặc kho Hàn), gồm: địa chỉ recipient, các `order_line` của recipient đó, `shipping_fee`, note `"web #<order_id> · kiện <i>/<N>"`, và gắn `pancake_customer_id` người đặt.
...
### 10.4 Theo dõi
- Mỗi đơn POS có `pancake_order_id`; POS webhook bắn trạng thái + vận đơn về Supabase, cập nhật `shipment.tracking_status/carrier/tracking_code`.
...
| 4 | Đặt VN, giao sang Hàn | VND | đều KR | kho Hàn, nội địa; phí ship Hàn quy đổi về VND | N ở nhánh Hàn |

Mọi ca: tiền tệ theo người đặt; kho + phí ship theo người nhận; một bill; theo dõi tách kiện. Dùng bảng này làm test khi build.
...
- Đo tỉ lệ chốt web-direct vs chat; sản phẩm chuẩn đẩy web, quà DN cao cấp giữ chat.
...
- **Backend/DB:** Supabase (bảng ở §4, RLS). Trạng thái đơn + giỏ ở server.
- **Config cần có:** `fx_rate(krw↔vnd)`; STK VN + STK Hàn theo currency; `warehouse.shipping_mode` + `fee_table` mỗi kho; Pancake Shop ID + API key (mỗi shop/kho) + webhook URL.
- **Fonts:** `next/font` load Playfair Display + Lora subset vietnamese.
- **Tiền tệ/i18n:** format theo `currency`; tách chuỗi để mở rộng ngôn ngữ.

---

## 14. Do / Don't

**Do:** tách 2 trục vùng ngay từ data model; một bill một tiền tệ theo người đặt; kho + ship theo người nhận; 1 web_order → N đơn Pancake; luôn bắt SĐT; radius nhỏ + UPPERCASE giãn chữ cho heritage; data-driven mọi thứ.

**Don't:** trộn tiền tệ trong một đơn; ship bánh vượt biên khi đã có kho vùng đó; nhồi giá từng bánh vào builder; nhét vùng vào SKU; localStorage cho giỏ ở production; sao chép logo/ảnh/chữ brand tham chiếu; hardcode tỉ giá.
...
- **Web-native (dashboard sở hữu):** nhóm đơn theo cùng khách (1 người đặt → N kiện → N đơn Pancake); doanh số hợp nhất chéo vùng + đa tiền tệ (quy về đơn vị đang chọn qua `fx_rate`); ngày muốn nhận + ghi chú ở mức quà/người nhận.
...
- **Các bảng:** KPI (doanh số theo range, số đơn, kiện theo vùng VN/Hàn, đang vận chuyển, cảnh báo tồn); doanh số theo ngày (hợp nhất); đơn theo khách (xổ ra từng đơn → từng kiện: người nhận, vùng, ngày giao, vận chuyển + vận đơn, ghi chú, ĐVVC, COD/trả trước, phí VC).
...
- **Bộ lọc:** khoảng thời gian (hôm nay/7 ngày/tháng), vùng (tất cả/VN/Hàn), tiền tệ hiển thị (₩/đ, quy đổi qua `fx_rate`), tìm theo tên/SĐT.
- **Tuyệt đối không** sửa tồn kho hay tạo/sửa đơn trong dashboard — mọi thay đổi ops đi qua Pancake để giữ một nguồn chân lý.
